...
public class Repository<DBEntity, ModelEntity> : IRepository<DBEntity, ModelEntity>
...

    {
        ...
        protected CasinoDbContext _context;
        protected ILogger _logger;
        protected readonly IMapper _mapper;
        public Repository(CasinoDbContext context, ILogger logger, IMapper mapper)
        {
            _mapper = mapper;
            _context = context;
            _logger = logger;
        ...
        public virtual async Task<ModelEntity> GetOneById(long id, string includeTables = "")
        {
            try
            {
                DBEntity query;
                if (String.IsNullOrEmpty(includeTables))
                {
                    query = await _set.AsNoTracking().FirstAsync(x => x.Id == id);
                }
                else
                {
                    query = await _set.Include(includeTables).AsNoTracking().FirstAsync(x => x.Id == id);
                }
